...
pdf.cell(200, 10, txt="Project Report: SnapClass AI Attendance System", ln=1, align='C')
pdf.ln(5)
...
- Artificial Intelligence (AI): Built using dlib and face_recognition_models for high-accuracy face detection and 128-dimensional facial embedding extraction. Uses scikit-learn Support Vector Classifier (SVC) for matching detected faces against the enrolled student database.
...
# Replace ascii chars that might cause problems or just multi_cell directly
for line in text.split('\n'):
    if line.strip().startswith('1.') or line.strip().startswith('2.') or line.strip().startswith('3.') or line.strip().startswith('4.') or line.strip().startswith('5.'):
        ...
        pdf.multi_cell(190, 8, txt=line.encode('latin-1', 'replace').decode('latin-1'))
        pdf.set_font("Arial", size=11)
    else:
        pdf.multi_cell(190, 6, txt=line.encode('latin-1', 'replace').decode('latin-1'))
